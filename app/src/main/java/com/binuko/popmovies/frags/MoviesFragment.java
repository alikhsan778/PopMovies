...
import com.binuko.popmovies.activities.MainActivity;
import com.binuko.popmovies.adapters.CoreAdapter;
import com.binuko.popmovies.extras.Keys;
...
import com.binuko.popmovies.utils.MoreListener;
import com.binuko.popmovies.views.ScaleInAnimationAdapter;
...
public class MoviesFragment extends Fragment {
        ...
        if (isNetworkAvailable()) {
            ...
            new CountDownTimer(400, 1000) {
                ...
                public void onFinish() {
                    if (conf.smallestScreenWidthDp >= 600) {
                        MainActivity.cornStamp.setVisibility(VISIBLE);
                        MainActivity.cornStamp.startAnimation(anim);
                    }
                }
            }.start();
        } else {
    ...
    private String mainUrl(int page, String sort) {
        return Url.BASE_URL + Url.PARAM_DISC + Url.PARAM_MOV + Url.PARAM_QUESTION_MARK
                + Url.PARAM_PAGE_NO + page + Url.PARAM_SORT + sort
                + Url.PARAM_ADULT + "false" + Url.PARAM_API + Url.KEY;
    }
            ...
            try {
                if (response != null) {
                    ...
                    for (int i = 0; i < arr.length(); i++) {
                        ...
                        Movie movie = new Movie();
                        movie.setId(jObj.getString(Keys.KEY_ID));
                        movie.setPoster(jObj.getString(Keys.KEY_POSTER));
                        movie.setBackdrop(jObj.getString(Keys.KEY_BACKDROP));
                        movie.setTitle(jObj.getString(Keys.KEY_TITLE));
                        movie.setOverview(jObj.getString(Keys.KEY_OVERVIEW));
                        movie.setDate(jObj.getString(Keys.KEY_DATE));
                        movie.setVoteAvr(jObj.getString(Keys.KEY_VOTE_AVR));
                        movies.add(movie);
                    }
                }
